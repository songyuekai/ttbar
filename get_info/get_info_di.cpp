...
void get_info_di(){
	...
	if(inputFile.Contains("TTTo2L2Nu")||inputFile.Contains("ttdilepton")){
  ...
  /////////////////////////////////////////////////////////////
	...
	Float_t M_tt_gen, delta_rapidity_gen, lep_charge[2];
	...
	if (inputFile.Contains("ttdilepton") ||
	  inputFile.Contains("TTTo2L2Nu")) {
		...
		mytree->Branch("lep_charge", lep_charge, "lep_charge[2]/F");
	...
	for (Int_t entry = 0; entry < chain.GetEntries(); entry++){
	    ...
	    if(inputFile.Contains("ttdilepton")||inputFile.Contains("TTTo2L2Nu")){
    	...
    	/*int lep_flag=0;
      	for (int i = 0; i < nlepton; i++) { // start from the second lepton
        	if (lepton_pt[i] > 30 && abs(lepton_eta[i]) < 2.4){
        		lepton_index[0]=i;
        		lep_flag++;
        		break;
        	}
      	...
      	if(lep_flag==1){
	      	for (int i = 0; i < nlepton; i++) {
	      		if (lepton_pt[i] > 20&& abs(lepton_eta[i]) < 2.4&& i!=lepton_index[0]){
	        		lepton_index[1]=i;
	        		lep_flag++;
	        		break;
	        	}
      	...
      	if(lep_flag==2){
      		for (int i = 0; i < nlepton; i++) {
	      		if (lepton_pt[i] > 15 && abs(lepton_eta[i]) < 2.4&& i!=lepton_index[0]&&i!=lepton_index[1]){
	        		lep_flag++;
	        		break;
	        	}
	      	}
	    ...
	    UInt_t index[45];
	    if(lepton_flag==true){
	    	int jet_num = 0; // count number fot jets satisfy the selection criteria
